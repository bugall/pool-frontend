...
export default {
            ...
            self.batch_sync_block(parseInt(current_block_height), length).then(function (blocks) {
                blocks.forEach(function (block, i) {
                    ...
                    block.transactions.forEach((transaction, j) => {
                        // let tx_id = block.transaction_ids[j]; // IDAChain based api will return transaction_ids as an array
                        ...
                        let tr_buffer = Buffer(JSON.stringify(transaction));
                        let tx_id = crypto.createHash('sha256').update(tr_buffer).digest('hex').substr(0, 40);
    ...
    batch_sync_block (start, length) {
        ...
        for (var i = 0; i < length; ++i) {
            promises.push(IDAChainService.fetch_block(parseInt(start) + i));
        }
    ...
    store () {
        return new Promise((resolve, reject) => {
            console.log(current_block_height, '############');
